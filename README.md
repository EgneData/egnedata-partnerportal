...
│   ├── domain/         api/           security/     ratelimit/
│   ├── audit/          purge/
│   └── PartnerPortalApplication.java
├── src/main/java/no/egnedata/datawallet/
│   ├── crypto/         envelope/      directory/    cli/
├── src/main/resources/
...
docker run -d --name partnerportal-pg \
  -e POSTGRES_DB=partnerportal \
...
SPRING_DATASOURCE_URL=jdbc:postgresql://localhost:5432/partnerportal \
SPRING_DATASOURCE_USERNAME=wallet_app \
SPRING_DATASOURCE_PASSWORD=devpw \
PARTNERPORTAL_WEB_ORIGIN=http://localhost:3000 \
java -jar target/partnerportal-*.jar
```
...
| `PARTNERPORTAL_WEB_ORIGIN` | no | CORS allow-list for the Flutter web client (e.g. `http://localhost:3000`) |
| `PARTNERPORTAL_ADMIN_OU` | no | X.509 OU required for admin mTLS (default `operators`) |
...
- **Java packages:** application → `no.egnedata.partnerportal.*`; key-exchange subsystem → `no.egnedata.datawallet.*`.
...
| `share` | Encrypt + sign + upload an envelope to a verifier |

The last three are dev-mode helpers. `register-verifier` works against any running server. `init-dev-trust` and `share --dev` need the server running with `bin/start.sh --dev` so the `it` profile + stub issuer principal resolver are wired (issuer mTLS off).
...
bin/cli.sh init-dev-trust \
  --jdbc-url jdbc:postgresql://localhost:5432/partnerportal \
  --jdbc-user wallet_app --jdbc-password --passphrase
bin/cli.sh share --url http://localhost:8443 \
