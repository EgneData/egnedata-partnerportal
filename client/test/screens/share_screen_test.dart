...
void main() {
  ...
  Future<({IssuerState state, Uint8List issuerKeyId})> buildIssuerState({
    ...
    final issuerRecord = encodedRecord(
    ...
    await installIdentity.save(installUuid, issuerRecord);
  ...
  DirectoryRecordWrapper verifierWrapper(Uint8List encodedRecord) =>
  ...
  group('ShareScreen happy path', () {
    testWidgets('builds envelope, posts it, navigates back on success',
      ...
      final (:state, :issuerKeyId) = await buildIssuerState(
        storage: storage,
        keystore: keystore,
      ...
      final verifierEncBytes = encodedRecord(
      ...
      when(() => mockDir.getIssuerDirectory(any()))
          .thenAnswer((_) async => [verifierWrapper(verifierEncBytes)]);
  ...
  group('ShareScreen verifier lookup failure', () {
    testWidgets(
      ...
      final (:state, issuerKeyId: _) = await buildIssuerState(
        storage: storage,
        keystore: keystore,
        installIdentity: installIdentity,
        bearerHttp: bearerHttp,
