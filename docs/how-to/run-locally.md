...
This page shows you how to boot the Partner Portal stack on your machine: one
...
1. Start (or reuse) the `partnerportal-pg` Docker container on port **5432**.
...
| `--dev` | Activates the `it` Spring profile and disables issuer mTLS, so test issuers are accepted without client certificates ([`bin/start.sh#L45-L51`](../../bin/start.sh#L45-L51)). Required for `init-dev-trust` and `share` workflows. |
...
| `--no-server` | Brings up Postgres only and exits; the server is not started ([`bin/start.sh#L126-L129`](../../bin/start.sh#L126-L129)). Used internally by `bin/start-e2e.sh`. |
...
| `POSTGRES_DB` | Database name created inside the container (default: `partnerportal`). |
...
| `PARTNERPORTAL_PG_CONTAINER` | Docker container name (default: `partnerportal-pg`). |
| `PARTNERPORTAL_PG_IMAGE` | Docker image pulled when creating a new container (default: `postgres:16`). |
| `PARTNERPORTAL_WEB_ORIGIN` | Value of the `Access-Control-Allow-Origin` header (default: `http://localhost:3000`). |
...
- [Run the BATS suite](run-bats.md) — run `bats/end_to_end.bats` through a
