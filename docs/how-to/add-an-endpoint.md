...
[`VerifierController.java`](../../src/main/java/no/egnedata/partnerportal/api/verifier/VerifierController.java)
is a good starting point. It shows the full pattern:

- `@RestController` + `@RequestMapping` at the class level
  ([lines 32–34](../../src/main/java/no/egnedata/partnerportal/api/verifier/VerifierController.java#L32-L34))
- Constructor injection of `AuditService` and a `@Value` property
  ([lines 43–49](../../src/main/java/no/egnedata/partnerportal/api/verifier/VerifierController.java#L43-L49))
- A POST handler consuming and producing `application/json`
  ([lines 51–92](../../src/main/java/no/egnedata/partnerportal/api/verifier/VerifierController.java#L51-L92))
- An `auditService.recordEvent(...)` call before returning
  ([lines 85–88](../../src/main/java/no/egnedata/partnerportal/api/verifier/VerifierController.java#L85-L88))

Its integration test lives at
[`VerifierControllerIT.java`](../../src/test/java/no/egnedata/partnerportal/api/verifier/VerifierControllerIT.java)
and shows the `@SpringBootTest` + `@Import(PostgresTestcontainer.class)` harness
([lines 25–29](../../src/test/java/no/egnedata/partnerportal/api/verifier/VerifierControllerIT.java#L25-L29)).
...
2. **Co-locate DTOs** in the same `api/<area>/` package — request bodies,
   ...
   [`VerifierRegistrationDto.java`](../../src/main/java/no/egnedata/partnerportal/api/verifier/VerifierRegistrationDto.java)
   as an example.
