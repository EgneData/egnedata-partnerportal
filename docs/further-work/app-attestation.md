...
not the Partner Portal server. The Partner Portal server doesn't know it
...
flowchart LR
    ...
    ext -- "verdict" --> ica
    ica -- "if verdict OK,<br/>sign directory record" --> ds[Partner Portal server]
```

The Partner Portal server's `DirectoryRecordVerifier` continues to verify
