...
the v1 pilot.
**Scope:** how to extend Partner Portal so that every install of a
...
sequenceDiagram
    ...
    participant Srv as Partner Portal server
...
trust anchor's signature**. Functionally, it is the Partner Portal
...
flowchart LR
    ...
    signed -- POST /v1/admin/directory<br/>application/cbor --> srv[Partner Portal server]
...
[eis]: ../../src/main/java/no/egnedata/partnerportal/api/entry/EntryIngestService.java
[drv]: ../../src/main/java/no/egnedata/datawallet/directory/DirectoryRecordVerifier.java
...
A separate Spring application (not in the Partner Portal server JAR)
...
This service authenticates to the Partner Portal server via admin mTLS
...
2. **Read from the Partner Portal server on demand** via
   ...
   Partner Portal server's availability.
...
- **Rebuilt at startup** by querying the Partner Portal server for all
...
- Bearer history / nonce log beyond the brief in-memory PoP-nonce
  challenge window.
- Audit chain (the Partner Portal server's `audit_log` is authoritative).
...
synchronisable across replicas via a periodic pull from the Partner
Portal server's revoked-rows view.
...
flowchart LR
    subgraph datacenter[Production deployment]
        subgraph host1[Host A]
            ds[Partner Portal server<br/>Spring Boot — this repo]
        end
...
|---|---|---|
| **Partner Portal server** (this repo) | The existing Spring Boot app | Knows nothing about an HSM. Still only sees admin-mTLS clients posting CBOR to `/v1/admin/directory`. From its point of view the intermediate is just *one operator* that publishes a lot of records. |
| **Intermediate signing service** | A *separate* Spring Boot deployable on a *different* host, with its own config and lifecycle | Hosts the enrollment, attestation, and bearer-JWT mint endpoints. Has no Postgres role in the Partner Portal database; talks to the server over the same admin-mTLS surface a human operator would use. |
...
- **Different blast radius.** A bug in the Partner Portal server (CBOR
...
- **Different uptime profile.** The intermediate is the bottleneck
  for *new install enrollment*. The Partner Portal server is the hot
  path for *every share*. Scale, deploy, and roll back them
  independently.
- **Different auth posture.** The intermediate authenticates *to* the
  Partner Portal server with admin mTLS. Merging the two would dissolve
...
- **Cross-device account migration is a separate problem.** If the
  ...
  work outside this design — the Partner Portal server has no concept of
  "user above install".
