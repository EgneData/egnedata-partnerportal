# Partner Portal v1 Pilot — Implementation Handoff
...
The Partner Portal v1 Pilot is a complete end-to-end encrypted envelope management system consisting of three components:
...
- **Crypto:** `src/main/java/no/egnedata/datawallet/crypto/`
...
- **Data model:** `src/main/java/no/egnedata/partnerportal/domain/`
...
- **Envelope codec:** `src/main/java/no/egnedata/datawallet/envelope/`
...
- **Directory & root quorum:** `src/main/java/no/egnedata/datawallet/directory/`
...
- **HTTP API:** `src/main/java/no/egnedata/partnerportal/api/`
...
- **Security:** `src/main/java/no/egnedata/partnerportal/security/`
...
- **Rate limiting & lockout:** `src/main/java/no/egnedata/partnerportal/ratelimit/`
...
- `SPRING_DATASOURCE_URL` — JDBC connection string (e.g. `jdbc:postgresql://localhost/partnerportal`)
- `SPRING_DATASOURCE_USERNAME` — DB user (e.g. `wallet_app`)
- `SPRING_DATASOURCE_PASSWORD` — DB password
- `PARTNERPORTAL_WEB_ORIGIN` — allowed CORS origin (production only)
- `PARTNERPORTAL_ADMIN_OU` — X.509 OU for admin mTLS (default: `operators`)
- `PARTNERPORTAL_SECURITY_ISSUER_MTLS` — set `false` to switch to bearer auth
- `PARTNERPORTAL_SECURITY_ADMIN_MTLS` — set `false` to open admin endpoints (dev only)
...
2. **Generate trust-root offline**
   ```bash
   bin/cli.sh gen-root --count 3 --threshold 2 --output pinned-root.cbor
   ```
3. **Publish root and directory** — sign directory records with offline root keys
4. **Deploy server** — PostgreSQL + Spring Boot app, load pinned root via `pinned_root_history` and configure issuer mTLS certs
