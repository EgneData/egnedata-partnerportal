# Partner Portal — API contract (companion to `plan.md`)
...
Issuer authentication: **mTLS** (default) or **bearer JWT** (mobile profile). Exactly one mode is active per deployment, controlled by `partnerportal.security.issuer-mtls` and `partnerportal.security.issuer-bearer.enabled` (mutually exclusive; both `true` is a startup error).
