#!/usr/bin/env bash
#
# Start the Partner Portal stack locally:
#   1. Postgres in Docker (skipped if container already exists/running)
#   2. Server JAR (skipped if target/partnerportal-*.jar exists)
...
PG_CONTAINER="${PARTNERPORTAL_PG_CONTAINER:-partnerportal-pg}"
PG_IMAGE="${PARTNERPORTAL_PG_IMAGE:-postgres:16}"
PG_DB="${POSTGRES_DB:-partnerportal}"
PG_USER="${POSTGRES_USER:-wallet_app}"
PG_PASSWORD="${POSTGRES_PASSWORD:-devpw}"
PG_PORT="${POSTGRES_PORT:-5432}"

WEB_ORIGIN="${PARTNERPORTAL_WEB_ORIGIN:-http://localhost:3000}"
...
JAR_GLOB="target/partnerportal-*.jar"
...
if [ "$DEV" -eq 1 ]; then
    ...
    JVM_ARGS+=("-Dpartnerportal.security.issuer-mtls=false")
fi
...
exec env \
    ...
    PARTNERPORTAL_WEB_ORIGIN="$WEB_ORIGIN" \
