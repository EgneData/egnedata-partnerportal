...
public class RepublishScheduler {
    ...
    public RepublishScheduler(PartnerPortalAdminClient adminClient,
    ...
    void doRepublish() {
        List<PartnerPortalAdminClient.ActiveIssuer> issuers = adminClient.getAllActiveIssuers(batchSize);
        log.info("Republish cycle: {} active issuers fetched", issuers.size());

        for (PartnerPortalAdminClient.ActiveIssuer issuer : issuers) {
            try {
                republishOne(issuer);
            } catch (Exception e) {
    ...
    private void republishOne(PartnerPortalAdminClient.ActiveIssuer issuer) {
