package no.egnedata.datawallet.intermediate.republish;
...
import no.egnedata.datawallet.intermediate.signer.SoftSigner;
...
class RepublishSchedulerTest {
    ...
    void republishesEligibleIssuerWithFreshIssuedAt() {
        ...
        PartnerPortalAdminClient.ActiveIssuer issuer = new PartnerPortalAdminClient.ActiveIssuer(
    ...
    void publishFailureDoesNotAbortRemainingIssuers() {
        ...
        PartnerPortalAdminClient.ActiveIssuer issuer1 = new PartnerPortalAdminClient.ActiveIssuer(
        ...
        PartnerPortalAdminClient.ActiveIssuer issuer2 = new PartnerPortalAdminClient.ActiveIssuer(
    ...
    void paginatesThroughMultiplePages() {
        ...
        List<PartnerPortalAdminClient.ActiveIssuer> page1 = new ArrayList<>();
        List<PartnerPortalAdminClient.ActiveIssuer> page2 = new ArrayList<>();
        ...
        for (int i = 0; i < 2; i++) {
            ...
            page1.add(new PartnerPortalAdminClient.ActiveIssuer(
        ...
        for (int i = 0; i < 2; i++) {
            ...
            byte[] keyId = DirectoryRecordSigner.computeKeyId(pubkey);
            byte[] record = originalSigner.signIssuerRecord(uuid, pubkey, keyId, "active");
            page2.add(new PartnerPortalAdminClient.ActiveIssuer(
                    uuid.toString(), B64URL.encodeToString(keyId),
                    ORIGINAL_TIME.toEpochMilli(),
                    ORIGINAL_TIME.plusMillis(365L * 24 * 3600 * 1000).toEpochMilli(),
                    ORIGINAL_TIME.toEpochMilli(),
    ...
    private static class RecordingClient extends PartnerPortalAdminClient {
    ...
    private static class FailFirstClient extends PartnerPortalAdminClient {
        final List<byte[]> published = new ArrayList<>();
        private final List<ActiveIssuer> issuers;
    ...
    private static class PaginatingClient extends PartnerPortalAdminClient {
        final List<byte[]> published = new ArrayList<>();
